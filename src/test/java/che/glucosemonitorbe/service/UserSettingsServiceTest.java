...
class UserSettingsServiceTest {
    ...
    @Test
    void getUserSettingsCreatesDefaultsWhenMissing() {
    ...
    @Test
    void saveUserSettingsUpdatesExistingEntity() {
        UUID userId = UUID.randomUUID();
        UserSettings existing = new UserSettings(userId, 2.0, 1.0, 45, 240);
        ...
        UserSettingsDTO result = service.saveUserSettings(userId, request);
    ...
    void deleteAndExistsDelegateToRepository() {
        ...
        boolean exists = service.hasUserSettings(userId);
     ...
     *
     * This test verifies that getUserSettings does NOT call repository.save when invoked.
     ...
     */
    @Test
    void c1_getUserSettings_mustHandleConcurrentInsertGracefully() {
        UUID userId = UUID.randomUUID();

        UserSettings existingSettings = new UserSettings(userId);
        ...
        assertThatNoException()
                ...
                .isThrownBy(() -> service.getUserSettings(userId));
    }

    @Test
    void l3_getUserSettings_mustNotCallRepositorySave() {
        UUID userId = UUID.randomUUID();
        when(repository.findByUserId(userId)).thenReturn(Optional.empty());
