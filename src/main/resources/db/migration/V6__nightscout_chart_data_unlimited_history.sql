...
ALTER TABLE nightscout_chart_data DROP COLUMN row_index;

CREATE UNIQUE INDEX uk_nightscout_chart_user_nsid
    ON nightscout_chart_data (user_id, nightscout_id)
    WHERE nightscout_id IS NOT NULL AND btrim(nightscout_id) <> '';

CREATE UNIQUE INDEX uk_nightscout_chart_user_ts
    ON nightscout_chart_data (user_id, date_timestamp)
    WHERE nightscout_id IS NULL OR btrim(nightscout_id) = '';

CREATE INDEX idx_nightscout_chart_user_date ON nightscout_chart_data (user_id, date_timestamp);
